...
class API(object):
    ...
    def _build_dump_url(self, method, **args):
        dump_args = dict((k,v,) for k,v in args.iteritems() if not k=='date')
        return self._build_url(method, **dump_args)
        ...
        if status == 200:
            try:
                ...
                if self.make_dumps:
                    ...
                    filename = md5(self._build_dump_url(method, format='json', **args)).hexdigest() + '.json'
...
class MockAPI(API):
    ...
    def call(self, method, **args):
        # Build the url
        url = self._build_dump_url(method, format='json', **args)
