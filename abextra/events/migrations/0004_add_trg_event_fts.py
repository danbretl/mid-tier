...
class Migration(SchemaMigration):
    tsv_func = 'event_ft_trigger()'

    def _reflect(self, orm):
        meta = orm.Event._meta
        table = meta.db_table
        search_vector_field, title_field, description_field = map(
            meta.get_field, ('search_vector', 'title', 'description')
        )
        search_vector_column, title_column, description_column = map(
            lambda f: f.db_column or f.column,
            (search_vector_field, title_field, description_field)
        )
        trigger_name = '_'.join((table, search_vector_column, 'update', 'trg'))
        return dict(
            table_name=table,
            index_column=search_vector_column,
    ...
    models = {
        ...
        'events.event': {
            ...
            'secret_key': ('django.db.models.fields.CharField', [], {'default': "'55ec8f6c7d'", 'max_length': '10', 'blank': 'True'}),
